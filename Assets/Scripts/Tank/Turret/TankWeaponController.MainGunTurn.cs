...
public partial class TankWeaponController : MonoBehaviour
{
    ...
    private void RotateHardware()
    {
        ...
        float targetYaw = GetTargetTurretYaw(localTargetDirection);
        Quaternion targetTurretRotation = GetTurretLocalRotationForYaw(targetYaw);
        ...
        // 应用旋转 (限制在硬性物理范围内)
        Quaternion targetBarrelRotation = GetBarrelLocalRotationForPitch(resolvedTargetPitch);
    ...
    private void RotateHardwareByMouseDelta()
    {
        ...
        float currentYaw = GetCurrentTurretYaw();
    ...
    private float GetCurrentTurretYaw()
    {
        Quaternion relativeRotation = turret.localRotation * Quaternion.Inverse(_turretBindLocalRotation);
        return NormalizeSignedAngle(relativeRotation.eulerAngles.y);
    }

    private float GetCurrentBarrelPitch()
    {
        Quaternion relativeRotation = barrel.localRotation * Quaternion.Inverse(_barrelBindLocalRotation);
        return NormalizeSignedAngle(relativeRotation.eulerAngles.x);
    }
    ...
    private Quaternion GetTurretLocalRotationForYaw(float yawDegrees)
    {
        return Quaternion.Euler(0f, yawDegrees, 0f) * _turretBindLocalRotation;
    }

    private Quaternion GetBarrelLocalRotationForPitch(float pitchDegrees)
    {
        return Quaternion.Euler(pitchDegrees, 0f, 0f) * _barrelBindLocalRotation;
    }

    private Quaternion GetBarrelRootWorldRotationForPitch(float pitchDegrees)
    {
        if (barrelRoot == null)
        {
        ...
        Transform parentTransform = barrelRoot.parent;
        Quaternion parentRotation = parentTransform != null ? parentTransform.rotation : Quaternion.identity;
        return parentRotation * Quaternion.Euler(pitchDegrees, 0f, 0f) * _barrelRootBindLocalRotation;
    }
